...
function Header({ onLoginClick, cartCount, wishlistCount, toggleWishlist }) {
  ...
  return (
    <div>
      ...
      {navCard === "eyeglasses" && 
      <div className = "absolute top-[28.3vh] px-10 flex right-10 z-50 h-[72vh] left-10 bg-white">
        <div className = "w-1/5 flex mt-20 flex-col">
          <button
          onClick={() => {setGender(0); setCategory(classiceyeglasses)}}
          className = {`${gender === 0 && "bg-orange-400/10"} py-4 px-8 rounded-md hover:bg-orange-400/10 items-center justify-between flex h-18 cursor-pointer`}><img className = "h-full" src = "/navpics/men.webp"/>Men<img className = "h-1/3" src = "/navpics/rightarrowblack.svg"/></button>
          <button
          onClick={() => {setGender(1); setCategory(classiceyeglasses)}}
          className = {`${gender === 1 && "bg-orange-400/10"} py-4 px-8 rounded-md hover:bg-orange-400/10 items-center justify-between flex h-18 cursor-pointer`}><img className = "h-full" src = "/navpics/women.webp"/>Women<img className = "h-1/3" src = "/navpics/rightarrowblack.svg"/></button>
          <button
          onClick={() => {setGender(2); setCategory(kidsclassiceyeglasses)}}
          className = {`${gender === 2 && "bg-orange-400/10"} py-4  rounded-md hover:bg-orange-400/10 items-center justify-between px-8 flex h-18 cursor-pointer`}><img className = "h-full" src = "/navpics/kids.webp"/>Kids<img className = "h-1/3" src = "/navpics/rightarrowblack.svg"/></button>
        </div>
        {(gender === 0 || gender === 1) && <div className = "flex flex-col">
          <h1 className = "h-18 flex items-center pl-4 border-b border-black/10 mb-2">SELECT CATEGORY</h1>
          <div className = "flex flex-col text-lg font-thin">
            <button
            onClick = {() => {setCategory(classiceyeglasses)}}
            className = {`${category === classiceyeglasses && "bg-orange-400/10"} justify-center px-8 rounded-md hover:bg-orange-400/10 items-start flex flex-col h-18 cursor-pointer`}>
              <p className = "text-lg">CLASSIC EYEGLASSES</p>
              <p className = "">Starting from Rs.2000</p>
            </button>
            <button
            onClick = {() => {setCategory(premiumeyeglasses)}}
            className = {`${category === premiumeyeglasses && "bg-orange-400/10"} justify-center px-8 rounded-md hover:bg-orange-400/10 items-start flex flex-col h-18 cursor-pointer`}>
              <p className = "text-lg">PREMIUM EYEGLASSES</p>
              <p className = "">Starting from Rs.4000</p>
            </button>
            <button
            onClick = {() => {setCategory(screeneyeglasses)}}
            className = {`${category === screeneyeglasses && "bg-orange-400/10"} justify-center px-8 rounded-md hover:bg-orange-400/10 items-start flex flex-col h-18 cursor-pointer`}>
        ...
        {gender == 2 && <div className = "flex flex-col">
          ...
          <div className = "flex flex-col text-lg font-thin">
            ...
            className = {`${category === kidsclassiceyeglasses && "bg-orange-400/10"} justify-center px-8 rounded-md hover:bg-orange-400/10 items-start flex flex-col h-18 cursor-pointer`}>
              <p className = "text-lg">Classic Eyeglasses</p>
        ...
        {category?.map((ceg, index) => {
          return (
            <div className = "flex flex-col w-40">
              <h1 className = "h-18 flex items-center pl-2 border-b border-black/10 mb-2">{ceg.name}</h1>
              {ceg.array.map((link, linkindex) => {
                return <a className = "text-sm text-neutral-400 pl-2 hover:text-[rgba(0,0,80,1)]" href = {link.url}>{link.name}</a>
              })}
            </div>
          )
        })}
      ...
      <div className = "absolute top-[28.3vh] px-10 flex right-10 z-50 h-[72vh] left-10 bg-white">
        ...
        {(gender === 0 || gender === 1) && <div className = "flex flex-col">
          ...
          <div className = "flex flex-col text-lg font-thin">
            <button
            className = {`justify-center px-8 rounded-md hover:bg-orange-400/10 items-start flex flex-col h-18 cursor-pointer`}>
        ...
        {category?.map((ceg, index) => {
          return (
            <div key = {index} className = "flex flex-col w-40">
              <h1 className = "h-18 flex items-center pl-2 border-b border-black/10 mb-2">{ceg.name}</h1>
              {ceg.array.map((link, linkindex) => {
                return <a key = {linkindex} className = "text-sm text-neutral-400 pl-2 hover:text-[rgba(0,0,80,1)]" href = {link.url}>{link.name}</a>
              })}
            </div>
          )
        })}
      ...
      {navCard === "kidsglasses" && 
      <div className = "absolute top-[28.3vh] px-10 items-center justify-evenly flex right-10 z-50 h-[72vh] left-10 bg-white">
          <button className = "w-68 h-68 justify-center rounded-xl flex flex-col items-center bg-zinc-100/50"><img src = "/navpics/eyeglasses.webp"/>Eyeglasses</button>
          <button className = "w-68 h-68 justify-center rounded-xl flex flex-col items-center bg-zinc-100/50"><img src = "/navpics/screenglasses.webp"/>Zero Power Screen glasses</button>
          <button className = "w-68 h-68 justify-center rounded-xl flex flex-col items-center bg-zinc-100/50"><img src = "/navpics/sunglasses.webp"/>Sunglasses</button>
      </div>}
        ...
        {category?.map((ceg, index) => {
          return (
            <div key = {index} className = "flex flex-col gap-2 w-48">
              <h1 className = "h-18 flex items-center text-sm pl-2 border-b border-black/10 mb-2">{ceg.name}</h1>
              {ceg.array.map((link, linkindex) => {
                return <a key = {linkindex} className = "text-sm text-neutral-400 pl-2 hover:text-[rgba(0,0,80,1)]" href = {link.url}>{link.name}</a>
              })}
            </div>
      ...
      {navCard === "eyetest" && 
        <div className = "absolute top-[28.3vh] flex right-10 z-50 h-[72vh] left-10 bg-white">
          ...
          <div className = "flex flex-col items-center text-center justify-evenly">
            ...
            <button className = "p-3 rounded-full text-lg border border-[rgba(0,0,80,1)]">Book Appointment</button>
          </div>
        </div>
      }
      ...
      {navCard === "storelocator" && 
        <div className = "absolute top-[28.3vh] flex right-10 z-50 h-[72vh] left-10 bg-white">
          <div className = "flex flex-col items-center text-center justify-evenly">
            ...
            <button className = "p-3 rounded-full text-lg border border-[rgba(0,0,80,1)]">Locate a store</button>
          </div>
